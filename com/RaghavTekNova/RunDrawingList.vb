...
Namespace RaghavTekNova
    ...
    <ComVisible(True)>
    <Guid("C29B7D4E-6E3F-4A8E-A135-82F8D2345678")> ' Replace with your own unique GUID
    <ProgId("RaghavTekNova.RunDrawingList")>
    Public Class RunDrawingList
        ...
        Public Sub V2_RUN_PR_Drawing_Tracking_list_VB()
            ...
            Try
                ...
                wsSource = CType(workbook.Sheets("Sheet10"), Worksheet)  ' Sheet10
                wsTarget = CType(workbook.Sheets("Sheet11"), Worksheet)  ' Sheet11

                ' Find the last row in the target sheet (Sheet11)
                ...
                ' Define the starting row in Sheet10 for pasting data
                targetStartRow = 8

                ' Clear existing data in the source sheet (Sheet10)
                ...
                With wsSource
                    ...
                    ' .Range("F5").Value = "TOTAL DAWING WT.(KG):-"
                ...
                wsSource.Range("A1:K1,A6:K6").Interior.Color = RGB(248, 203, 173)
                wsSource.Range("A7:K7").Interior.Color = RGB(173, 216, 230)
                ' wsSource.Range("F5").Interior.Color = RGB(226, 239, 218)
                ' wsSource.Range("F5").Interior.Color = RGB(0, 176, 240)
                wsSource.Range("A6").Interior.Color = RGB(248, 203, 173)
                ' wsSource.Range("H5").Interior.Color = RGB(169, 208, 142)

                ' Clear contents before merging to avoid data loss warning
                wsSource.Range("A1:K1,A6:K6").ClearContents()
                wsSource.Range("A2:B5").ClearContents()
                wsSource.Range("D2:I2,D3:I3,D4:I4").ClearContents()
                wsSource.Range("D5:E5,F5:G5,H5:I5").ClearContents()

                ' Merge and format cells for project headers and details
                wsSource.Range("A1:K1,A6:K6").Merge()

                With wsSource.Range("A1:K1")
                    .Value = "BILL OF MATERIAL (DRAWING TRACKING LIST)"
                ...
                wsSource.Range("D5:E5").Merge()
                wsSource.Range("F5:G5").Merge()
                wsSource.Range("H5:I5").Merge()
                ' wsSource.Range("J2:K5").Merge()

                ' Write headers in row 7
                wsSource.Range("A7").Value = "SR.NO."
                wsSource.Range("B7").Value = "DRAWING NO."
                wsSource.Range("C7").Value = "ASSEMBLY NAME"
                wsSource.Range("D7").Value = "ASSEMBLY MARK"
                wsSource.Range("E7").Value = "QTY."
                wsSource.Range("F7").Value = "UNIT WEIGHT"
                wsSource.Range("G7").Value = "TOTAL WEIGHT"
                wsSource.Range("H7").Value = "REVISION"
                wsSource.Range("I7").Value = "DATE"
                wsSource.Range("J7").Value = "DESCRIPTION"
                wsSource.Range("K7").Value = "REMARK"

                ' Format header row
                With wsSource.Range("A7:K7")
                    .Font.Bold = True
                    .HorizontalAlignment = XlHAlign.xlHAlignCenter
                    .VerticalAlignment = XlVAlign.xlVAlignCenter
                    .Font.Size = 12
                ...
                ' Adjust column widths (fixed width except C, J, K)
                wsSource.Columns("A").ColumnWidth = 8    ' SR.NO.
                wsSource.Columns("B").ColumnWidth = 25   ' DRAWING NO.
                ' wsSource.Columns("D").ColumnWidth = 20   ' ASSEMBLY MARK
                wsSource.Columns("E").ColumnWidth = 8    ' QTY.
                wsSource.Columns("F").ColumnWidth = 15   ' UNIT WEIGHT
                wsSource.Columns("G").ColumnWidth = 15   ' TOTAL WEIGHT
                wsSource.Columns("H").ColumnWidth = 12   ' REVISION
                wsSource.Columns("I").ColumnWidth = 12   ' DATE

                ' Copy data from the target sheet (Sheet11) to the source sheet (Sheet10)
                dataRange = wsTarget.Range("C8:L" & lastRow)
                dataRange.Copy(wsSource.Cells(targetStartRow, 2)) ' Paste data starting from column B

                ' Add serial numbers in column A of the source sheet (Sheet10)
                ...
                ' Align content from row 7 onward to the left
                With wsSource.Range("A7:K" & wsSource.Rows.Count)
                ...
                ' Autofit columns C, D, H, J, and K for varying text length
                wsSource.Columns("C").AutoFit()
                wsSource.Columns("D").AutoFit()
                wsSource.Columns("H").AutoFit()
                wsSource.Columns("J").AutoFit()
                wsSource.Columns("K").AutoFit()
                ...
                wsSource.Range("D2").Value = wsTarget.Range("D2").Value
                wsSource.Range("D3").Value = wsTarget.Range("D3").Value
                wsSource.Range("D4").Value = wsTarget.Range("D4").Value
                wsSource.Range("D5").Value = wsTarget.Range("D5").Value
                ...
                ' === Force Sheet10 as active to avoid wrong window context ===
                workbook.Sheets("Sheet10").Select() ' ✅ Stay on Sheet10
                Dim win As Object = excelApp.ActiveWindow

                ' === Clean up UI ===
                win.DisplayGridlines = False               ' Hide gridlines (Sheet10)
                win.DisplayHeadings = False                ' Hide row/column labels
                excelApp.DisplayFormulaBar = True         ' Show formula bar
                win.DisplayWorkbookTabs = True            ' Show sheet tabs
                ...
                MsgBox("Drawing tracking list formatted successfully!", MsgBoxStyle.Information)
        ...
        Private Sub SetPrintPreferences(app As Application)
            ...
            Try
                ...
                ws.PageSetup.PrintArea = "$A$1:$K$" & lastRow + 1
