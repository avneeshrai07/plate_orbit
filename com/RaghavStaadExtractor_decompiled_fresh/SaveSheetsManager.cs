// ===== RaghavStaadExtractor.SaveSheetsManager =====

// Decompiled member-by-member from RaghavStaadExtractor.dll



public SaveSheetsManager()
{
}
...
private Application GetRunningExcel()
{
	...
	catch (Exception projectError)
	{
		...
		try
		{
			Application result = (Application)Activator.CreateInstance(Marshal.GetTypeFromCLSID(new Guid("00024500-0000-0000-C000-000000000046")));
			ProjectData.ClearProjectError();
			return result;
		}
		catch (Exception ex)
...
private void SafeDeleteShapes(Worksheet ws)
{
	...
	try
	{
		...
		try
		{
			enumerator = ws.Shapes.GetEnumerator();
