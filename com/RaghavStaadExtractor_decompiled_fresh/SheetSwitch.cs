// ===== RaghavStaadExtractor.SheetSwitch =====
...
private string PromptForPassword(string prompt, string title)
{
	...
	form2.Font = new Font("Segoe UI", 9f);
	...
	checked
	{
		...
		MouseEventHandler value = delegate(object sender, MouseEventArgs e)
		{
		...
		MouseEventHandler value2 = delegate(object sender, MouseEventArgs e)
		{
			if (flag)
			{
				Point location2 = form.Location;
				location2.X += e.X - location.X;
				location2.Y += e.Y - location.Y;
				form.Location = location2;
			}
		};
		MouseEventHandler value3 = delegate
		{
			flag = false;
		};
