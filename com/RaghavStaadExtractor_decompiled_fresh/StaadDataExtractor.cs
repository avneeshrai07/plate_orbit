// ===== RaghavStaadExtractor.StaadDataExtractor =====
...
public string ExtractBeamDataFromStaad()
{
	...
	try
	{
		...
		if (obj2 == null)
		{
			return "STAAD is not running or OpenSTAAD not available.";
		}
		...
		string result = ProcessBeamData(RuntimeHelpers.GetObjectValue(objectValue), RuntimeHelpers.GetObjectValue(obj2));
	...
	catch (Exception ex)
	{
		...
		Exception ex2 = ex;
		string result2 = "Error: " + ex2.Message;
...
private string ProcessFrameAnalysis(object ws, object staadApp)
{
	checked
	{
		try
		{
			...
			if ((Operators.CompareString(text, "X", TextCompare: false) != 0) & (Operators.CompareString(text, "Z", TextCompare: false) != 0) & (Operators.CompareString(text, "G", TextCompare: false) != 0))
			{
				return $"Invalid prompt '{text}' in AW2. Please enter 'X', 'Z', or 'G' only.";
			}
			if (Operators.CompareString(text, "G", TextCompare: false) == 0)
			{
				return ProcessBothDirectionsForGridSilent(RuntimeHelpers.GetObjectValue(ws), RuntimeHelpers.GetObjectValue(staadApp));
			}
			if (!AreGridsDefined(text, RuntimeHelpers.GetObjectValue(ws)))
			{
				return string.Format("FOR FRAME ANALYSIS DEFINE GRIDS FIRST USING 'G' OPTION{0}{1}Grid data for '{2}' direction is not found Nova Grid System.{3}{4}Please follow these steps:{5}1. Enter 'G' in cell AW2{6}2. Run the analysis to generate grid system{7}3. Then use '{8}' for frame analysis", "\r\n", "\r\n", text, "\r\n", "\r\n", "\r\n", "\r\n", "\r\n", text);
			}
			List<BeamData> list = new List<BeamData>();
			object instance;
			object[] array;
			bool[] array2;
			object instance2 = NewLateBinding.LateGet(ws, null, "Cells", array = new object[2]
			{
			...
			if (num < 9)
			{
				return "No beam data found for frame analysis.";
			}
			int num2 = num;
			for (int i = 9; i <= num2; i++)
			{
				...
				object[] obj = new object[2] { i, 2 };
				array = obj;
				bool[] obj2 = new bool[2] { true, false };
				array2 = obj2;
				instance = NewLateBinding.LateGet(ws, null, "Cells", obj, null, null, obj2);
				if (array2[0])
				{
					i = (int)Conversions.ChangeType(RuntimeHelpers.GetObjectValue(array[0]), typeof(int));
				}
			...
			if (list.Count == 0)
			{
				return "No valid beam data found for frame analysis.";
			}
			List<BeamData> list2 = FilterBeamsForDefinedGridsOnly(list, text, RuntimeHelpers.GetObjectValue(ws));
			if (list2.Count == 0)
			{
				return $"No beams found in predefined grids for {text} direction analysis. Check your grid definition using 'G' option first.";
			}
			...
			return string.Format("Grid-Restricted Frame Analysis for '{0}' direction:{1}{2}{3}{4}{5}", text, "\r\n", text3, "\r\n", "\r\n", text2);
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
			Exception ex2 = ex;
			string result = $"Frame analysis error: {ex2.Message}";
			ProjectData.ClearProjectError();
			return result;
		}
	}
...
private string ProcessBothDirectionsForGridSilent(object ws, object staadApp)
{
	checked
	{
		try
		{
			...
			if (num < 9)
			{
				return "No beam data found for grid generation.";
			}
			int num2 = num;
			for (int i = 9; i <= num2; i++)
			{
			...
			if (list.Count == 0)
			{
				return "No valid beam data found for grid generation.";
			}
			...
			return string.Format("NOVA GRID SYSTEM GENERATED SUCCESSFULLY!{0}{1}Grid data has been created in NovaGrid System:{2}", "\r\n", "\r\n", "\r\n") + string.Join("\r\n", list2) + string.Format("{0}{1}", "\r\n", "\r\n") + string.Format("You can now perform frame analysis using 'X' or 'Z' options.{0}", "\r\n") + "Analysis will be restricted to these predefined grids only.";
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
			Exception ex2 = ex;
			string result = $"Grid generation error: {ex2.Message}";
			ProjectData.ClearProjectError();
			return result;
		}
	}
...
private bool AreGridsDefined(string direction, object ws)
{
	try
	{
		...
		instance = NewLateBinding.LateGet(obj, null, "Range", array = new object[1] { text2 }, null, null, array2 = new bool[1] { true });
		if (array2[0])
		{
		...
		if (NewLateBinding.LateGet(instance, null, "Value", new object[0], null, null, null) != null)
		{
			...
			object[] obj4 = new object[1] { text2 };
			array = obj4;
...
private Dictionary<string, List<BeamData>> CreateGridSystem(List<BeamData> beamDataList, string direction, object ws)
{
	...
	List<double> list = dictionary2.Keys.OrderBy((double x) => x).ToList();
	...
	checked
	{
		...
		else
		{
			List<double> list2 = (from c in list
				...
				select c).ToList();
			List<double> list3 = list.Where((double c) => Math.Abs(c) < 0.005).ToList();
...
{
	Sheet4GridData result = default(Sheet4GridData);
	result.GridNames = new List<string>();
	result.AllCoordinates = new List<double>();
...
private void WriteCoordinateSpacingToSheet4(List<double> sortedCoordinates, string direction, object ws)
{
	checked
	{
		try
		{
			...
			instance = NewLateBinding.LateGet(obj, null, "Range", array = new object[1] { text }, null, null, array2 = new bool[1] { true });
			if (array2[0])
			{
				text = (string)Conversions.ChangeType(RuntimeHelpers.GetObjectValue(array[0]), typeof(string));
			}
...
private string AnalyzeSimilarFrames(List<BeamData> beamDataList, string direction, object ws)
{
	...
	Dictionary<string, List<BeamData>> dictionary2 = dictionary.Where((KeyValuePair<string, List<BeamData>> g) => g.Value.Count >= 2).ToDictionary((KeyValuePair<string, List<BeamData>> k) => k.Key, (KeyValuePair<string, List<BeamData>> v) => v.Value);
	...
	if (Operators.CompareString(direction, "G", TextCompare: false) != 0)
	{
		...
		string text = (_lastLogFilePath = SaveLogFile(content, direction, RuntimeHelpers.GetObjectValue(ws)));
...
private string GetGridNamingInfoForResult(string direction, object ws)
{
	try
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			if (GetCompleteGridDataFromSheet4(direction, RuntimeHelpers.GetObjectValue(ws)).AllCoordinates.Count > 0)
			{
				return $"From Nova's Grid System: '{text}' (Coordinate-matched)";
			}
			return $"From Nova's Grid System: '{text}'";
		}
		return "default naming (Nova's Grid System is empty)";
	}
	catch (Exception projectError)
	{
		ProjectData.SetProjectError(projectError);
		string result = "default naming (Nova's Grid System not available)";
		ProjectData.ClearProjectError();
		return result;
	}
}
...
private bool AreFramesSimilar(List<BeamData> frame1, List<BeamData> frame2, string direction)
{
	...
	if (Operators.CompareString(direction, "Z", TextCompare: false) == 0)
	{
		num = list.Min((BeamData b) => Math.Min(b.XA, b.XB));
		num2 = list.Min((BeamData b) => Math.Min(b.YA, b.YB));
		num3 = list2.Min((BeamData b) => Math.Min(b.XA, b.XB));
		num4 = list2.Min((BeamData b) => Math.Min(b.YA, b.YB));
	}
	else
	{
		...
		num = list.Min((BeamData b) => Math.Min(b.YA, b.YB));
		num2 = list.Min((BeamData b) => Math.Min(b.ZA, b.ZB));
		num3 = list2.Min((BeamData b) => Math.Min(b.YA, b.YB));
		num4 = list2.Min((BeamData b) => Math.Min(b.ZA, b.ZB));
	}
	checked
	{
		...
		{
			BeamData beam = list3[k];
			BeamData beam2 = list4[k];
			...
			if (Operators.CompareString(direction, "Z", TextCompare: false) == 0)
			{
				num8 = beam.ZB - beam.ZA;
				num9 = beam2.ZB - beam2.ZA;
			}
			else
			{
				num8 = beam.XB - beam.XA;
				num9 = beam2.XB - beam2.XA;
			}
			if (Math.Abs(num8 - num9) > 0.01)
			{
				return false;
			}
		}
...
private string GenerateEnhancedFrameAnalysisLog(List<List<KeyValuePair<string, List<BeamData>>>> similarGridGroups, Dictionary<string, List<BeamData>> allGridGroups, string direction, object ws = null)
{
	...
	checked
	{
		...
		for (int i = 0; i <= num; i++)
		{
			...
			string text2 = string.Join(", ", list.Select((KeyValuePair<string, List<BeamData>> g) => g.Key.Split('(')[0].Trim()));
			if (text2.Length > 45)
			{
		...
		stringBuilder.AppendLine(new string('-', 80));
		foreach (KeyValuePair<string, List<BeamData>> item in allGridGroups.OrderBy((KeyValuePair<string, List<BeamData>> g) => ExtractCoordinateFromGridName(g.Key)))
		{
		...
		{
			List<KeyValuePair<string, List<BeamData>>> list2 = similarGridGroups[j];
			stringBuilder.AppendLine($"\ud83c\udfd7\ufe0f SIMILAR GRID GROUP #{j + 1}");
			stringBuilder.AppendLine();
			...
			{
				KeyValuePair<string, List<BeamData>> keyValuePair = list2[k];
				stringBuilder.AppendLine($"\ud83d\udcd0 {keyValuePair.Key} DETAILS:");
				stringBuilder.AppendLine();
				foreach (BeamData item3 in keyValuePair.Value.OrderBy((BeamData b) => b.BeamNo))
				{
			...
			if (j < similarGridGroups.Count - 1)
			{
				stringBuilder.AppendLine("=" + new string('=', 80));
				stringBuilder.AppendLine();
			}
		...
		{
			num4 += similarGridGroup.Count;
			foreach (KeyValuePair<string, List<BeamData>> item4 in similarGridGroup)
			{
				num5 += item4.Value.Count;
			}
		}
		stringBuilder.AppendLine($"Total Grids with Similar Patterns: {num4}");
		stringBuilder.AppendLine($"Total Beams in Similar Grids: {num5}");
		if (num4 > 0)
		{
			string arg = ((double)num5 / (double)num4).ToString("F1");
		...
		string arg2 = ((allGridGroups.Count > 0) ? ((double)num4 / (double)allGridGroups.Count * 100.0).ToString("F1") : "0");
...
private string SaveLogFile(string content, string direction, object ws)
{
	...
	catch (Exception ex)
	{
		...
		catch (Exception ex3)
		{
			ProjectData.SetProjectError(ex3);
			Exception ex4 = ex3;
			string result = $"Error saving log file: {ex4.Message}";
			ProjectData.ClearProjectError();
			return result;
		}
	}
...
private object GetStaadApplication()
{
	try
	{
		return Marshal.GetActiveObject("StaadPro.OpenSTAAD");
	}
	catch (Exception projectError)
	{
		ProjectData.SetProjectError(projectError);
...
private object GetOrCreateWorksheet(object workbook, string sheetName)
{
	try
	{
		...
		bool[] obj2 = new bool[1] { true };
		bool[] array2 = obj2;
		object result = NewLateBinding.LateGet(workbook, null, "Sheets", obj, null, null, obj2);
		if (array2[0])
		{
...
private void FormatExcelWorksheet(object ws)
{
	...
	checked
	{
		...
		object[] array2;
		bool[] array3;
		object instance2 = NewLateBinding.LateGet(ws, null, "Cells", array2 = new object[2]
		{
			NewLateBinding.LateGet(instance = NewLateBinding.LateGet(ws, null, "Rows", new object[0], null, null, null), null, "Count", new object[0], null, null, null),
			1
		...
		if (num2 >= 9)
		{
			...
			object obj3 = (obj2[1] = 22);
			object[] array6 = obj2;
			bool[] obj4 = new bool[2] { true, false };
			bool[] array7 = obj4;
			object obj5 = NewLateBinding.LateGet(ws, null, "Cells", obj2, null, null, obj4);
			if (array7[0])
			{
...
private void ProcessSectionFarmingAndWeight(object ws)
{
	...
	checked
	{
		for (int i = 9; i <= num2; i++)
		{
			...
			else
			{
				...
				else if (unchecked(num4 == 613 || num4 == 614 || num4 == 615))
				{
					...
					unchecked
					{
						...
						case 614:
						{
							object[] obj25 = new object[2] { i, 26 };
							array = obj25;
						...
						case 615:
						{
							object[] obj29 = new object[2] { i, 38 };
							array = obj29;
						...
						case 613:
						{
							object[] obj39 = new object[2] { i, 26 };
							array = obj39;
						...
						case 614:
						{
							object[] obj37 = new object[2] { i, 28 };
							array = obj37;
						...
						case 615:
						{
							object[] obj35 = new object[2] { i, 26 };
							array = obj35;
				...
				else
				{
					...
					if (num4 == 680)
					{
						object[] obj43 = new object[2] { i, 24 };
						array = obj43;
					...
					else
					{
						...
						do
						{
							object[] obj45 = new object[2] { i, num41 };
							array = obj45;
							bool[] obj46 = new bool[2] { true, true };
							array2 = obj46;
					...
					{
						object[] obj47 = new object[2] { i, 37 };
						array = obj47;
						bool[] obj48 = new bool[2] { true, false };
						array2 = obj48;
			...
			if (Conversion.Val(RuntimeHelpers.GetObjectValue(NewLateBinding.LateGet(obj5, null, "Value", new object[0], null, null, null))) == 0.0)
			{
				object[] obj53 = new object[2] { i, 43 };
				array = obj53;
			...
			{
				object[] obj57 = new object[2] { i, 11 };
				array = obj57;
			...
			if (num57 == 0.0)
			{
				object[] obj61 = new object[2] { i, 37 };
				array = obj61;
...
[SpecialName]
[CompilerGenerated]
private double _Lambda$__39-1(KeyValuePair<string, List<BeamData>> g)
{
	return ExtractCoordinateFromGridName(g.Key);
}
